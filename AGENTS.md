...
## Estado vigente del proyecto (2026-08-26, source/HEAD `0.30.8`)
...
- **Audio:** final `0.30.7` baseline accepted by the maintainer after human playtesting; levels, mix, and crossfade are closed for this release. The canonical pack reconstructs 50 events / 97 variants from 96 versioned masters. No diagnostic counters or quantitative route data were supplied or claimed.
- **Next sequence:** completed human runs plus `pnpm stats` for balance/retention → package the committed `0.30.8` candidate → external Steamworks publication/icon confirmation, production achievement-unlock smoke, and technical close.
- **Delivery state:** source/HEAD declares `0.30.8`. `DEV_TOOLS.shortMaps = false`, `audioDiagnostics = false`, `mapTransitionKey = false`, and `finaleKey = false`; the candidate is ready for packaging after commit/validation, but **no `0.30.8` package is claimed yet**. Recording, FPS, and gold rigs are clean.
