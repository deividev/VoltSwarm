...
## Estado vigente del proyecto (2026-08-26, source/HEAD `0.30.9`)
...
- **Audio:** final `0.30.7` baseline accepted by the maintainer after human playtesting; levels, mix, and crossfade are closed for this release. The canonical pack reconstructs 50 events / 97 variants from 96 versioned masters. No diagnostic counters or quantitative route data were supplied or claimed. Provider receipts, generation URLs/IDs, account/subscription records, and private commercial-entitlement evidence for the shipped Suno/ElevenLabs masters are outside the Steam launch gate by maintainer-owned risk acceptance; runtime provenance remains, and no legal-entitlement proof is claimed.
- **Next sequence:** package the exact committed `0.30.9` bytes → private SteamPipe upload and production achievement-unlock smoke → external Steamworks publication/icon confirmation and technical close.
- **Delivery state:** source/HEAD `0.30.9` is committed; the `0.30.8` installer/portable remain the previous packaged baseline. The retired packaged audio-benchmark route is removed, the internal benchmark now runs only through Vite DEV, and the document/native title is `Voltswarm`. `DEV_TOOLS.shortMaps = false`, `audioDiagnostics = false`, `mapTransitionKey = false`, and `finaleKey = false`. No `0.30.9` package is claimed yet; packaging follows validation of the committed bytes. Recording, FPS, and gold rigs are clean.
