...
| `docs/ACHIEVEMENTS.md` | Catálogo canónico de 20 logros, metadatos Steam, estado App Admin y arquitectura de desbloqueo con `steamworks.js`. |
...
## Estado vigente del proyecto (2026-08-27, source/HEAD `0.30.14`)
...
- **Achievements:** catálogo de lanzamiento **20/20 implementado** en `ACHIEVEMENT_REGISTRY`; **Steam achievement transport** implementado con `steamworks.js` `0.4.0`, IPC aislado, allowlist y outbox persistente/idempotente. SDK init, App ID, `electronEnableSteamOverlay`, packaging nativo e IPC son soporte auxiliar de ese flujo, NO features Steamworks independientes. El mantenedor confirma **20/20 entradas creadas** en Steamworks App Admin para App ID `4979220`. Git no prueba publicación de los últimos cambios, subida de iconos ni smoke end-to-end del desbloqueo en build de producción.
- **Otras APIs Steamworks:** Leaderboards, User Stats, Cloud, Workshop, Rich Presence, Friends/lobbies/networking, Steam Input, Inventory/DLC/microtransactions y cualquier otra integración NO están implementadas y quedan **fuera del alcance de lanzamiento**. Solo se reconsideran post-lanzamiento si Voltswarm demuestra suficiente visibilidad/tracción; no están confirmadas ni prometidas y no bloquean RC/cierre Steam.
- **Audio:** final `0.30.7` baseline accepted by the maintainer after human playtesting; levels, mix, and crossfade are closed for this release. The canonical pack reconstructs 50 events / 97 variants from 96 versioned masters. The retained `0.30.9` DEV benchmark honestly failed its original ≤22 voice policy at 24; on 2026-08-27 the maintainer accepted 24 as the non-blocking ceiling because performance, load-failure, leak and cleanup checks passed, and future policy rejects >24. This is internal DEV regression/load evidence, not production/Steam/minimum-hardware proof. No diagnostic counters or quantitative route data were supplied or claimed. Provider receipts, generation URLs/IDs, account/subscription records, and private commercial-entitlement evidence for the shipped Suno/ElevenLabs masters are outside the Steam launch gate by maintainer-owned risk acceptance; runtime provenance remains, and no legal-entitlement proof is claimed.
- **Next sequence:** package the exact committed `0.30.14` bytes only if a new package is requested.
- **Delivery state:** source/HEAD `0.30.14` is committed; the `0.30.13` installer/portable remain the latest packaged exact-byte baseline. The retired packaged audio-benchmark route is removed, the internal benchmark runs only through Vite DEV, and the document/native title is `Voltswarm`. `DEV_TOOLS.shortMaps = false`, `audioDiagnostics = false`, `mapTransitionKey = false`, and `finaleKey = false`. `0.30.14` is not packaged. Recording, FPS, and gold rigs are clean.
...
- **Current full-game sequence:** package the exact committed `0.30.14` bytes only if a new package is requested. `0.30.13` remains the latest packaged exact-byte baseline, and `0.30.14` is not packaged. Audio cohesion is closed for `0.30.7`. The 20 launch achievements and their `steamworks.js` Steam achievement transport are implemented, and their 20 App Admin entries are maintainer-confirmed as created; do not turn that into a claim that changes are published/live or production-validated. SDK/overlay initialization and packaging are auxiliary achievement infrastructure, not separate product features. The three launch characters, the current Foundry replacement slice (Furnace Mite, Axle Runner, Slagcaster), and the current Foundry ambient visual state are closed; molten-flow glow and voxel sparks are optional future enhancements, while deferred enemy expansion is not part of this sequence. Hazard Marshal already has its baseline closed in 0.22.0; fine balance and an optional reactive arena remain deferred. The Demo does not extend this arc. **The multiplayer feasibility gate remains deferred to post-launch by user decision**; multiplayer/co-op is neither implemented nor announced. **All other Steamworks product APIs are outside launch scope and carry no commitment; reconsider only post-launch if visibility/traction justifies them.** **Internal Demo RC target:** end of August 2026; this is not a public date, availability promise, or review confirmation. External claims still require current confirmation.
