...
| Efecto visual / shader / particulas / sonido | `docs/DIRECCION_ARTE.md` + `docs/REFERENCIAS_VISUALES.md` | VFX = cubos voxel de paleta; cero gore; validar con 400+ enemigos |
| Cualquier sonido/SFX/música | `docs/SOUND_EVENT_CATALOG.md` (leyes de estilo + estado por evento) → `docs/MUSIC_PROMPTS.md` (provenance Suno) → `docs/AUDIO_AUTHORING_PIPELINE.md` | 6 leyes 2026-07-18: NADA retro · latencia cero (precarga total, sync a constantes reales de animación) · frecuente=invisible (ruido, oscuro, 4 micro-variantes) · asimetría disparo/muerte · pirámide de volumen (arma>pickups>muerte) · muerte=VFX de cubos. Veredicto final SIEMPRE in-game |
...
| `docs/SOUND_EVENT_CATALOG.md` | REESCRITO 2026-07-18: las 6 leyes de estilo de audio + catálogo completo de eventos con estado real (12 integrados / 10 hooks mudos / resto Fase 5). Fuente de verdad del audio. |
| `docs/SOUND_DIRECTION.md` | ⚠️ SUPERSEDED: dirección "juguete industrial" vieja — pendiente de reescritura a las leyes actuales; ante conflicto manda `SOUND_EVENT_CATALOG.md`. |
| `docs/MUSIC_PROMPTS.md` | Provenance Suno: prompt ancla "Neon Horizon" (118 BPM moderno), criterios de selección de tomas, direcciones rechazadas. Toda música debe rastrear a una entrada aquí. |
...
- **Foundation de audio — GRAN AVANCE 2026-07-18/19:** estilo ENCONTRADO y validado in-game. **2026-07-19: 10 hooks mudos CERRADOS (batches A-D) + música de menú + primer lote de armas propias (Pulse/Press/Ricochet; hooks `WEAPON_FIRE_SFX` para las 11).** ~28 eventos sonando. Quedan 7 armas (blades/welder continuas=difíciles), procs de mods, peligros/ataques de enemigos-boss, ambiente, camas del álbum — Fase 5, briefs en `SOUND_EVENT_CATALOG.md`. QoL: pausa con RIG+stats, números XP/oro acumuladores, sliders con valor, click UI universal, sección de Sockets en Unlocks (dev). **Pendiente con oídos frescos: revisar muerte de enemigo (fatiga auditiva del usuario — no tocar en caliente) + pasada de cohesión/balance global.**
- **Siguiente secuencia:** rematar foundation de audio (hooks mudos + camas) → gate interno de viabilidad multijugador → Fase 5 (contenido, balance, retención y diferenciación de personajes) → catálogo completo de audio → Fase 6 (Steamworks y cierre). Multiplayer/co-op no está implementado ni anunciado; el gate puede terminar NO-GO. **Stretch interno agresivo:** 2026-11-04 para el scope congelado (sin host migration/dedicated/hybrid, exactamente 3 personajes, ~30 contratos y sin expansión no planificada); no es fecha segura ni promesa pública y se revalida tras Audio Foundation + GO/NO-GO. Coming Soon sigue dependiente de la aprobación de Steam. Leaderboards: confirmados para el juego completo, todavía no implementados.
...
Sesión de búsqueda de estilo completada (17 rondas de prototipos validados 1 a 1 in-game). Resultado: **12 eventos sonando** (bolt "brrt" eléctrico, muerte = explosión de cubitos, pickups XP/oro, UI universal con listener delegado, level-up 2 tiempos, cofre 3 actos sincronizado a la bezier real, tienda, música "Neon Horizon" provisional) servidos desde `public/assets/audio/prototypes/manifest.json` + `AUDIO.validation.enabledEvents`. Las 6 leyes de estilo viven en `docs/SOUND_EVENT_CATALOG.md`; generadores DSP deterministas en `tools/audio/prototype-*.mjs` (motor `dsp.mjs`); ElevenLabs disponible (`elevenlabs-sfx*.mjs`, key en `.env` gitignored). Teclas de audición F2-F9 ciclan candidatos in-game (TEMP `AUDIO.validation.auditionKeys`). El pack viejo `assets/audio/sfx/` sigue siendo fixture rechazado (`paths.finalManifest`) — NO borrarlo aún. **Menú**: "Neon Swarm (4)" cableada (`menu-music`, arranca al primer gesto, handover al darle Play). **Álbum/DLC**: carpetas por pista en `tmp/music-prototypes/` con `PROMPT.md` cada una; plan en `docs/MUSIC_PROMPTS.md`. **Limpieza 2026-07-19**: prototypes a 35 archivos vigentes; en tools/audio solo dsp.mjs + generadores r9/r13/r15/r17 + elevenlabs-sfx-v2 + pipeline original. Siguiente: 10 hooks mudos → llenar camas del álbum → catálogo completo en Fase 5 (briefs en `SOUND_EVENT_CATALOG.md`).
