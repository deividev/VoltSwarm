...
## Estado vigente del proyecto (2026-08-26, source/HEAD `0.30.7`)
...
- **Next sequence:** completed human runs plus `pnpm stats` for balance/retention → package the committed `0.30.7` candidate → external Steamworks publication/icon confirmation, production achievement-unlock smoke, and technical close.
- **Delivery state:** source/HEAD declares `0.30.7`. `DEV_TOOLS.shortMaps = false`, `audioDiagnostics = false`, `mapTransitionKey = false`, and `finaleKey = false`; the candidate is ready for packaging after commit/validation, but **no `0.30.7` package is claimed yet**. Recording, FPS, and gold rigs are clean.
