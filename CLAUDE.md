...
## Estado vigente del juego completo — source/HEAD 0.30.18 (2026-08-31)
...
- **Audio:** final levels, mix, and crossfade baseline accepted by the maintainer after human playtesting. The canonical pack reconstructs 50 events / 97 variants from 96 versioned masters. The retained `0.30.9` DEV run remains an honest FAIL under its original ≤22 voice policy; on 2026-08-27 the maintainer accepted 24 as the non-blocking ceiling because performance, load-failure, leak and cleanup checks passed, and future policy rejects >24. This is not production/Steam/minimum-hardware proof. The acceptance does not invent diagnostic counters or quantitative route data that were not supplied.
- **Remaining Steam launch work:** clean prior build output, package the exact committed `0.30.18` bytes, and verify setup, portable, and ASAR payload. Source `0.30.18` excludes the four obsolete, unreferenced Rack Hauler v3 seafoam sheets from Electron packaging while retaining the active v4 runtime sheets. No `0.30.18` package is claimed until external packaging evidence exists. `shortMaps=false`, `audioDiagnostics=false`, `mapTransitionKey=false`, and `finaleKey=false`.
