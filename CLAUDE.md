...
## Estado vigente del juego completo — source/HEAD 0.30.14 (2026-08-27)
...
- **Remaining Steam launch work:** package the exact committed `0.30.14` bytes only if a new package is requested. `0.30.13` remains the latest packaged exact-byte baseline; `0.30.14` is not packaged. `shortMaps=false`, `audioDiagnostics=false`, `mapTransitionKey=false`, and `finaleKey=false`.
