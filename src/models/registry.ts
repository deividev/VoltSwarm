...
export interface VoxelModelDef {
  ...
  /** Fraction of the side sheet that is the BODY, from its back edge. Set it
   *  when the sheet also draws an appendage pointing along +Z (the Slagcaster
   *  cannon): per-row depth is one symmetric number, so an uncropped sheet
   *  turns that appendage's LENGTH into torso fat. See the voxelizer option. */
  sideProfileBodyFraction?: number;
  /** RECT of the front sheet (fractions of its bbox, y downward) holding an
   *  appendage that geometry supplies. Cleared before extrusion so it is not
   *  blown up to body depth. Must be a rect, not a column cut: on the
   *  Slagcaster sheet the same columns carry the cannon AND the right leg. */
...
export const VOXEL_MODELS: Record<string, VoxelModelDef> = {
  ...
  slagcaster: {
    kind: 'enemy',
    ref: 'assets/2d/ref-slagcaster-deployed-front-v3.png',
    sideProfileRef: 'assets/2d/ref-slagcaster-deployed-side-v3.png',
    backPaintRef: 'assets/2d/ref-slagcaster-deployed-back-v3.png',
    sidePaint: true,
    asymmetric: true,
    // The cannon is drawn in all three sheets, and it is the one part that
    // cannot come FROM them: per-row depth is a single symmetric number, so an
    // uncropped side sheet turns the barrel's length into torso fat (rows 9..19
    // extruded 48 deep against a ~32-deep body). These two windows hand the
    // cannon to geometry and leave the rest of every sheet untouched.
    //
    // MEASURED. Corrected side-v3 sheet: body columns 0..26 of 47, cannon
    // begins at column 27 and reaches 46. Front
    // sheet: cannon at columns 36..54 of 55, ROWS 6..20 of 37 — the rows matter,
    // because below row 21 those same columns are the right leg, and cutting by
    // column alone amputated it.
    sideProfileBodyFraction: 27 / 47,
    frontAppendageRect: { fromX: 36 / 55, fromY: 6 / 37, toY: 21 / 37 },
    targetWidth: 55,
    voxelSize: 0.030273,
    ...
    // With sideProfileRef the depth per row is MEASURED, so these bands only
    // set the left-right falloff, not the volume.
    segments: [
  ...
  'slagcaster-closed': {
    ...
    sidePaint: true,
    asymmetric: true,
    // 27 @ 0.045 -> 40 @ 0.030273 (2026-08-22). This endpoint's GRID is
    // preview-only — the runtime ball is generated by projecting the deployed
    // vertices onto a sphere — so the number that actually ships is the
    // DIAMETER the transform reads from here, and 40 x 0.030273 = 1.211 holds
    // the old 1.215 (and SLAGCASTER.rollingRadius) to within 0.4%. Matching
    // the deployed endpoint's pitch is the point: both poses are one material.
    targetWidth: 40,
    voxelSize: 0.030273,
    bodyColor: SLAGCASTER_OLIVE,
    palette: [SLAGCASTER_OLIVE, DARK, SLAGCASTER_AMBER],
...
export async function buildModelGrid(key: string): Promise<VoxelGrid> {
  ...
  const grid = def.refSide
    ...
    : await voxelizeIcon(def.ref, {
        ...
        sideProfileBodyFraction: def.sideProfileBodyFraction,
        frontAppendageRect: def.frontAppendageRect,
