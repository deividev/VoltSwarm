...
**REGLA NUEVA (2026-07-06): a partir de ahora, generar SIEMPRE 3 vistas por personaje — frontal, lateral y trasera —, no solo frontal.** Motivo histórico: el entonces boss final de prueba (`final-boss` en `registry.ts`, ref `ref-volt-warden-front.png` sin `-v2`) se ve bien de frente pero es una masa lisa sin detalle de lado y de espaldas — la técnica de extrusión algorítmica (segments/depthFactor) solo puede ADIVINAR la profundidad real sin una vista lateral que la confirme. Hazard Marshal conserva hoy `final-boss`; las fuentes de Volt Warden permanecen como referencia histórica. Con las 3 vistas, la lateral fija el perfil de profundidad real (en vez de la aproximación elíptica actual) y la trasera evita que la espalda quede en blanco o repita la cara.
