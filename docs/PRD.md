...
### Snapshot vigente — source candidate 0.30.9 (2026-08-26)
...
| Delivery | Source candidate `0.30.9`; `0.30.8` remains the previous packaged baseline. The packaged audio-benchmark route is removed, the internal benchmark is Vite-DEV-only, and the document/native title is `Voltswarm`. `shortMaps=false`, `audioDiagnostics=false`, `mapTransitionKey=false`, and `finaleKey=false`; no `0.30.9` package is claimed before review and commit. |

**Next sequence:** review/commit the `0.30.9` release-hardening candidate → package those exact bytes → private SteamPipe upload and production achievement-unlock smoke → external Steamworks publication/icon confirmation and technical close. Audio cohesion, the achievement catalog and Steam achievement transport are closed implementation work; external confirmation and RC validation are not. Other Steamworks APIs are not part of this sequence or the launch scope. Closed characters, the current Foundry visual state, and deferred enemy expansion are not active work items.
...
4. 🟡 **NOW:** the full arc is playable, the Hazard Marshal baseline is closed in 0.22.0, and audio cohesion is closed in 0.30.7 after maintainer playtesting. The 20 launch achievements and their `steamworks.js` Steam achievement transport are implemented; the maintainer confirms 20/20 matching App Admin entries created. Next: review/commit/package the 0.30.9 release-hardening candidate → private SteamPipe upload → production achievement-unlock smoke and external Steamworks publication/icon confirmation. Every other Steamworks product API is explicitly outside launch scope. All three launch characters, the current Foundry visual state, and the current Foundry replacement slice are closed.
...
From `0.30.9`, `pnpm benchmark:audio` runs the same deterministic workload only through Vite DEV plus development Electron. The rig is isolated in `src/dev/audio-benchmark.ts`; production Electron has no CLI/query route, and `tools/check-release-bundle.mjs` rejects its tokens and full-Game hook in `dist/`/`electron/dist/`. Tooling-owned acceptance is mean FPS ≥60, every complete one-second bucket ≥50 FPS, p99 >0 and ≤33.4 ms, minimum/end population ≥400, peak voices ≤22, exercised kill/XP/Gold/audio paths, zero load failures/leaks and zero active voices after cleanup. Measurement starts only after a fixed 5 s rendered-frame warm-up under the full workload, then samples a fixed 10 s.

The single controlled post-policy run is preserved at `docs/evidence/audio-benchmark-dev-0.30.9.json` and **FAILED**: mean 105.20 FPS, minimum bucket 97 FPS, p99 32.7 ms, minimum/end population 434/448, 10 kills, 8 XP pickups and 21 Gold pickups passed, but peak voices reached 24 against the ≤22 bound. Load failures, leaks and cleanup active voices were zero. It was not retried or tuned. This is internal DEV regression/load evidence only, not packaged Steam performance or minimum-hardware proof.
