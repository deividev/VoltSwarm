...
- **Tiers (rareza) — DEFINICIÓN CANÓNICA. 5 tiers: gris → verde → azul → morado → dorado** (`Rarity` en `upgrades.ts`; pesos de tirada en `TIERS.weights`/`luckShift`, Luck sube los tiers altos). ⚠️ Cada categoría usa los tiers DISTINTO — esto es lo que hay que respetar para que no haya desalineamientos:
  ...
  - **Sustain (2026-08-10):** Nanobot Swarm cura `1/2/3/4/5` HP cada 5 s por tier; tanto la carta como la ficha lo presentan como `12/24/36/48/60 HP/min`, derivado de la misma configuración. Hull Plates aumenta solo el Max HP, sin curación inmediata, y la ficha muestra una fila `Max HP` con el total vivo del jugador. Leech Coil usa `2/3/4/7/10%` de probabilidad por golpe para curar 1 HP, con un cooldown global de 1 s entre curaciones.
