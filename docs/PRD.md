...
Todo personaje nuevo exige `modelKey` validado y frontal/lateral/trasera planas aprobadas; la frontal empaquetada es el retrato 2D sobre `#444e5e`/`#2b3340`. La banda de roster comunica thumbnail, nombre y estado: `Unlocked` o texto visible exacto `Locked` junto a `assets/2d/icon-ui-lock-v2.png`, sin emoji. El perfil integrado presenta retrato grande, encabezado, una fila con icono in-game veraz por cada stat (sin combinar stats ni usar Shield para Armor), firma config-derived, Recommended Weapon solo de presentación, tradeoff y Contract/progreso. Menú y pre-run comparten renderer, `data-character-*`, teclado/gamepad y bloqueo de Confirm. `.character-layout` es el único fallback de scroll de la sección; perfil y roster no crean scroll anidado y las acciones permanecen fijas. No se monta WebGL: `src/models/character-preview.ts` queda como infraestructura dormida. Field Engineer implementa este contrato con `ref-field-engineer-front-v1.png`.
...
- El menú **Characters** y la selección previa al arma usan el mismo renderer y contenido. La banda superior compacta envuelve los botones nativos; el perfil es un artículo sin caja interior, compuesto con separadores y raíles de acento. En el tier de encaje distribuye tres columnas: identidad con retrato grande, nombre, resumen y estado; las 9 filas completas del stat-sheet; y Signature, Recommended Weapon, Tradeoff más requisito/progreso exacto de Contract cuando aplica. Field Repair sigue config-backed al 1%, Bolt Cannon continúa solo como **Recommended Weapon** y el tradeoff declara `-5% Damage`. No se clampa, trunca ni oculta información. A 1920×1080 y 1024×600 el contenido actual cabe sin scrollbar; ventanas menores, copy futuro o roster con varias filas degradan a dos y una columna y a un único scroll vertical de `.character-layout`. `.character-grid` y el perfil usan overflow visible; Back/Confirm quedan fuera y siempre visibles. Teclado, mouse, XInput/DirectInput e Interact remapeado conservan el handoff en límites, incluso sin rango de scroll. El preview 3D permanece dormido y los atributos `data-character-*` no cambian.
