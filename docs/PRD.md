...
El balance compartido sigue gobernado por la autoridad común. Este corte solo introduce el experimento acotado de Lucky Gear Common (`+6% -> +4%`), aplicado de forma idéntica en `demo-map1` y `map-2`; cualquier otro cambio de balance debe validarse en la autoridad y propagarse explícitamente a ambas ramas derivadas.
...
## Estado de la arquitectura (actualizado 2026-08-06, v0.13.13-demo)
...
- **Sustain Cores**: Nanobot Swarm restaura `2/6, 4/6, 6/6, 8/6, 10/6` HP cada 10 s y muestra `2/4/6/8/10 HP/min` por tier. Cada copia suma su magnitud a la regeneración acumulada. Hull Plates conserva sus valores de Max HP: no modifica el HP actual ni activa Field Repair. Field Repair cura 1% del HP máximo tras instalar o subir cualquier otro Core durante gameplay. La hoja de stats muestra el Max HP total actual. Leech Coil otorga `0.25/0.5/1/1.5/2%` de probabilidad por tier de restaurar exactamente 1 HP al acertar, con un cooldown global de 1 segundo; sus copias acumulan esa probabilidad. Como el tier de cada Core se tira con pesos afectados por Luck, más Luck aumenta la probabilidad de obtener sus magnitudes altas, pero no altera la magnitud de una carta ya tirada.
...
- **Tiers (rareza) — DEFINICIÓN CANÓNICA. 5 tiers: gris → verde → azul → morado → dorado** (`Rarity` en `upgrades.ts`; pesos de tirada en `TIERS.weights`/`luckShift`, Luck sube los tiers altos). ⚠️ Cada categoría usa los tiers DISTINTO — esto es lo que hay que respetar para que no haya desalineamientos:
  - **Calibración base (playtest 2026-07-17; experimento 2026-08-04):** con `Luck = 0`, pesos por carta 62/27/9/1.8/0.2. En una pantalla de 3 cartas esto deja `5.881%` de ver al menos una morada/dorada y `0.599%` de ver una dorada. La primera copia Common de Lucky Gear baja de `+6%` a `+4%`: con ella las probabilidades pasan a aproximadamente `11.633%` y `2.857%`. Es el ÚNICO número de balance que cambia para el próximo playtest humano de la demo; Luck base sigue en `0%`, los tiers superiores siguen en `8/10/14/20%` y no cambian pesos, `luckShift`, fórmulas, consumidores, desbloqueos ni economía de cofres/Scrapper.
...
- **Audio de navegación UI**: hover del ratón y cambios reales de foco por teclado/Tab/mando emiten `ui-focus` oscuro y bajo, una vez por objetivo; el foco inicial/programático, controles ocultos/deshabilitados y repetidos no suenan. Confirm, Back, Resume y compra usan una sola señal semántica por acción; Escape y B conservan la acción contextual.
...
Todo personaje nuevo exige `modelKey` validado y frontal/lateral/trasera planas aprobadas; la frontal empaquetada es el retrato 2D sobre `#444e5e`/`#2b3340`. La banda de roster comunica thumbnail, nombre y estado: `Unlocked` o texto visible exacto `Locked` junto a `assets/2d/icon-ui-lock-v2.png`, sin emoji. El artículo integrado no repite ese estado y se ordena **Identity → Gameplay Identity → Stats**: Signature es protagonista, Tradeoff va inmediatamente debajo y **Suggested Start** es apoyo honesto que nunca garantiza el arma ni altera pool u odds. Las nueve stats conservan fila e icono veraz; solo valores distintos de `PLAYER`/`defaultStats()` reciben énfasis. Un bloqueo mantiene requisito y progreso exacto a ancho completo. Menú y pre-run comparten renderer, `data-character-*`, nombres completos, teclado/gamepad y bloqueo de Confirm. Breakpoints: `>=900px` tres columnas; `600–899px` identidad + gameplay y stats debajo; `420–599px` identidad compacta, gameplay apilado y stats a dos columnas; `<420px` una columna. `.character-layout` es el único posible dueño de scroll; roster, artículo, módulos y columnas usan overflow visible y las acciones permanecen fijas. La sección solo entra en el foco si tiene rango real: sin rango entrega directamente a Back/Confirm; con rango se recorre y sale por ambos límites. No se monta WebGL: `src/models/character-preview.ts` queda como infraestructura dormida. Field Engineer implementa este contrato con `ref-field-engineer-front-v1.png`; la tarjeta usa alt player-facing y el retrato repetido del perfil es decorativo porque la región conserva el nombre accesible.
...
- El menú **Characters** y la selección previa al arma usan el mismo renderer y contenido. La banda superior compacta envuelve los botones nativos; el perfil es un artículo sin caja interior con **Identity | Gameplay Identity | Stats**. Field Repair sigue config-backed al 1%, `-5% Damage` es el tradeoff inmediato y Bolt Cannon queda como **Suggested Start** secundario sin sinergia inventada. El renderer deriva énfasis de baseline: Max HP, Armor y Damage son `changed`; las otras seis stats permanecen legibles como baseline. El estado desbloqueado no se duplica y los bloqueos conservan requisito/progreso a ancho completo. `.character-layout` es el único posible scroll vertical y solo es focusable cuando existe rango; Back/Confirm quedan fuera y fijos. El preview 3D permanece dormido y los atributos `data-character-*` no cambian.
...
Los contratos de progreso con armas distintas muestran dentro del bloque de progreso una tira `COUNTED WEAPONS` con los iconos reales y nombres accesibles de los IDs que ya cumplen su condición. Arsenal enseña únicamente armas que alcanzaron el umbral de daño de maestría en el ledger de runs terminadas; probar un arma sin dominarla, abandonar una run o un ID desconocido no produce un icono ni aumenta el contador.
