...
- El menú **Characters** y la selección previa al arma usan el mismo renderer y el mismo contenido. Una banda superior compacta conserva botones nativos con thumbnail, nombre y estado; debajo, un único artículo integrado y sin marco compone **Identity | Gameplay Identity | Stats**. Signature/Field Repair es el módulo protagonista, Tradeoff aparece justo debajo y Bolt Cannon queda como **Suggested Start** secundario: solo se etiqueta si aparece naturalmente, sin garantía ni cambio de odds. Las nueve stats siguen visibles; el renderer compara cada valor con `PLAYER`/`defaultStats()` y marca como `changed` únicamente Max HP, Armor y Damage en el Field Engineer actual, dejando las seis baselines más silenciosas pero plenamente legibles. El estado seleccionado/desbloqueado no se repite en el artículo; un personaje bloqueado conserva requisito y progreso exacto de Contract a ancho completo. La frontal aprobada sigue siendo `ref-field-engineer-front-v1.png`, con alt player-facing solo en la tarjeta; el retrato repetido del perfil es decorativo y el artículo conserva nombre accesible mediante su región. Breakpoints finales: `>=900px` tres columnas; `600–899px` identidad + gameplay y stats en la fila siguiente; `420–599px` identidad compacta, gameplay apilado y stats a dos columnas; `<420px` una columna. `.character-layout` es el único posible dueño de overflow vertical; roster, artículo, columnas y módulos permanecen visibles, mientras Back/Confirm quedan fuera y fijos. La sección solo es focusable si `scrollHeight > clientHeight`: sin rango, teclado/gamepad pasan de roster a acciones; con rango, recorren toda la sección y salen por sus límites. No se crea canvas, observer ni WebGL, y los contratos `data-character-*` permanecen estables.
...
Hay **29 contratos declarados y 27 activos**. `Proving Ground` y `Run Completion` son definiciones latentes: no se evalúan ni se muestran. Ambos siguen declarando internamente un premio `next-core`; `Run Completion` evalúa runs completadas y no registra personajes distintos, por lo que ninguno debe describirse como progresión de personaje implementada.
...
- Los dos contratos latentes (`Proving Ground` y `Run Completion`) permanecen definidos, pero nunca se evalúan ni se muestran.
