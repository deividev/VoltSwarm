...
### Snapshot vigente — source/HEAD 0.30.18 (2026-08-31)
...
| Delivery | Source/HEAD `0.30.18` is committed. Electron packaging excludes only the four obsolete, unreferenced Rack Hauler v3 seafoam conversion sheets; the active v4 runtime sheets remain packaged. Exact-byte `0.30.18` packaging is the current delivery gate and is not claimed until external packaging evidence exists. The packaged audio-benchmark route is removed, the internal benchmark is Vite-DEV-only, and the document/native title is `Voltswarm`. `shortMaps=false`, `audioDiagnostics=false`, `mapTransitionKey=false`, and `finaleKey=false`. |

**Next sequence:** clean prior build output, package the exact committed `0.30.18` bytes, and verify setup, portable, and ASAR payload before claiming a new packaged baseline. Audio cohesion, the achievement catalog and Steam achievement transport are closed implementation work; external confirmation and RC validation are not. Other Steamworks APIs are not part of this sequence or the launch scope. Closed characters, the current Foundry visual state, and deferred enemy expansion are not active work items.
...
4. 🟡 **NOW:** the full arc is playable, the Hazard Marshal baseline is closed in 0.22.0, and audio cohesion is closed in 0.30.7 after maintainer playtesting. The 20 launch achievements and their `steamworks.js` Steam achievement transport are implemented; the maintainer confirms 20/20 matching App Admin entries created. Next: clean prior build output, package the exact committed 0.30.18 bytes, and verify setup, portable, and ASAR payload. Source `0.30.18` contains the bounded obsolete Rack Hauler v3 sheet exclusion, but no `0.30.18` package is claimed without external packaging evidence. Every other Steamworks product API is explicitly outside launch scope. All three launch characters, the current Foundry visual state, and the current Foundry replacement slice are closed.
...
- **Tiers (rareza) — DEFINICIÓN CANÓNICA. 5 tiers: gris → verde → azul → morado → dorado** (`Rarity` en `upgrades.ts`; pesos de tirada en `TIERS.weights`/`luckShift`, Luck sube los tiers altos). ⚠️ Cada categoría usa los tiers DISTINTO — esto es lo que hay que respetar para que no haya desalineamientos:
  ...
  - **Sustain (2026-08-29):** Nanobot Swarm cura `2/6, 4/6, 6/6, 8/6, 10/6` HP cada 10 s por tier; tanto la carta como la ficha lo presentan como `2/4/6/8/10 HP/min`, derivado de la misma configuración. Cada copia suma su magnitud a la regeneración acumulada. Hull Plates aumenta solo el Max HP: no modifica el HP actual ni activa Field Repair. Field Repair cura 1% del HP máximo tras instalar o subir cualquier otro Core durante gameplay. La ficha muestra una fila `Max HP` con el total vivo del jugador. Leech Coil usa `0.25/0.5/1/1.5/2%` de probabilidad por golpe y por tier para curar exactamente 1 HP, con un cooldown global de 1 s entre curaciones; sus copias acumulan esa probabilidad. Como el tier de cada Core se tira con pesos afectados por Luck, más Luck aumenta la probabilidad de obtener sus magnitudes altas, pero no altera la magnitud de una carta ya tirada.
...
- **Empaquetado Electron**: `pnpm package` genera instalador NSIS (`-setup.exe`, asistente + desinstalador) Y portable (`-portable.exe`, un archivo para testers) en `release/`. Authenticode/code signing is not a gate for the Steam-only release; SmartScreen "Unknown Publisher" remains an optional post-launch concern only for direct downloads outside Steam. **Regla de rutas de assets (mordió 3 veces el mismo día)**: en strings de JS/markup SIEMPRE relativas (`'assets/...'` — `file://` rompe las absolutas y Vite no puede reescribir strings); en CSS `url()` SIEMPRE absolutas (`'/assets/...'` — Vite las reescribe al compilar; las relativas resuelven contra `src/ui.css`). Gamepad API = Chromium nativo, cero cambios en el main process de Electron.
...
- `CHARACTER_REGISTRY['rack-hauler']` usa `modelKey: 'rack-hauler'`, portrait `ref-rack-hauler-front-v4-seafoam.png`, Open Rack config-derived y unlock `proving-ground`. El Contract está activo y concede `{ kind: 'character', id: 'rack-hauler' }` de forma idempotente sin reemplazar el array vivo de `PROFILE`.
- El modelo runtime consume frontal/lateral/trasera/top seafoam v4: `#BAE8C6`, tool green `#3B9B73`, graphite `#202830`, visor `#E9F6FF`. Las cuatro hojas se regeneran desde las fuentes ortográficas aprobadas y añaden un marco alpha deliberado, sin ampliar los raster v3; el voxelizador mide el bbox ocupado, así que conserva 14.914 vóxeles / 13.120 triángulos por instancia. Previews 0°/90°/180°/270° y capturas reales de Mapa 1/2 están verificadas. El harness read-only mediante hooks DEV seleccionó Rack Hauler real con desbloqueo solo en memoria y sostuvo 430 enemigos durante 12 s en Mapa 2: 430/430 movidos, modelo `rack-hauler` antes/después, 119.94 FPS medios, bucket mínimo 119.76, mediana 8.3 ms, p99 8.5 ms y 0 errores de página. Esto cierra su gate específico 400+ de lectura/modelo y ausencia de degradación obvia. No sustituye el benchmark canónico VFX-heavy de 65 s: `tools/perf-stress.mjs` siempre confirma Field Engineer y no acepta personaje; parametrizarlo queda como cobertura extendida, no como gate de Rack pendiente.
...
Historical `0.30.7/0.30.8` packaged benchmark evidence: deterministic `audio-swarm-416` (seed 4979220; digest `4979220:240-112-48:0.25:4`), 404 peak / 411 minimum / 411 end active enemies at 800x600 after 3 s warmup + 10 s rAF: 120.10 mean FPS, 119 minimum complete 1 s bucket FPS and 8.5 ms frame-time p99 on Windows 10 / AMD Ryzen 7 3700X / NVIDIA GeForce RTX 2060 (D3D11). Actual paths: 9 kills, 7 XP pickups, 14 Gold pickups; audio 47 attempts / 27 accepted, 15 peak voices, 20 cooldown drops, 0 steals/load failures/leaks and 0 active voices after cleanup. This remains historical machine/scenario evidence only.

From `0.30.9`, `pnpm benchmark:audio` runs the same deterministic workload only through Vite DEV plus development Electron. The rig is isolated in `src/dev/audio-benchmark.ts`; production Electron has no CLI/query route, and `tools/check-release-bundle.mjs` rejects its tokens and full-Game hook in `dist/`/`electron/dist/`. Current tooling-owned acceptance is mean FPS ≥60, every complete one-second bucket ≥50 FPS, p99 >0 and ≤33.4 ms, minimum/end population ≥400, peak voices ≤24, exercised kill/XP/Gold/audio paths, zero load failures/leaks and zero active voices after cleanup. Measurement starts only after a fixed 5 s rendered-frame warm-up under the full workload, then samples a fixed 10 s.

The single controlled post-policy run is preserved unchanged at `docs/evidence/audio-benchmark-dev-0.30.9.json` and honestly **FAILED** under its original ≤22 policy: mean 105.20 FPS, minimum bucket 97 FPS, p99 32.7 ms, minimum/end population 434/448, 10 kills, 8 XP pickups and 21 Gold pickups passed, but peak voices reached 24. Load failures, leaks and cleanup active voices were zero; it was not retried. On 2026-08-27, the maintainer accepted 24 simultaneous voices as the current non-blocking internal DEV ceiling because performance, load-failure, leak and cleanup checks all passed; future policy rejects >24. This is internal DEV regression/load evidence only, not packaged Steam performance or minimum-hardware proof.
...
Los contratos de progreso con armas distintas muestran dentro del bloque de progreso una tira `COUNTED WEAPONS` con los iconos reales y nombres accesibles de los IDs que ya cumplen su condición. Arsenal enseña únicamente armas que alcanzaron el umbral de daño de maestría en el ledger de runs terminadas; Proving Ground enseña las armas iniciales presentes en runs terminadas. Abandonar una run o un ID desconocido no produce un icono ni aumenta el contador.
