...
### Snapshot vigente — source/HEAD 0.30.14 (2026-08-27)
...
4. 🟡 **NOW:** the full arc is playable, the Hazard Marshal baseline is closed in 0.22.0, and audio cohesion is closed in 0.30.7 after maintainer playtesting. The 20 launch achievements and their `steamworks.js` Steam achievement transport are implemented; the maintainer confirms 20/20 matching App Admin entries created. Next: package the exact committed 0.30.14 bytes only if requested; `0.30.13` remains the latest packaged exact-byte baseline, and `0.30.14` is not packaged. Every other Steamworks product API is explicitly outside launch scope. All three launch characters, the current Foundry visual state, and the current Foundry replacement slice are closed.
