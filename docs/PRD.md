...
- [ ] **Un solo flujo:** Characters y la selección pre-run comparten renderer. Se preservan `data-character-*`, teclado/gamepad y el bloqueo de Confirm para personajes cerrados; `.character-layout` es el único fallback de scroll de toda la sección.
...
- El menú **Characters** y la selección previa al arma usan el mismo renderer y el mismo contenido. Una banda superior compacta y envolvente conserva botones nativos con thumbnail, nombre y estado; el perfil seleccionado es un artículo integrado sin caja interior, dividido mediante espaciado, separadores y raíles de acento. En el tier de encaje usa tres columnas: identidad con retrato grande, nombre, resumen y estado; las 9 filas completas del stat-sheet; y las reglas completas de Signature, Recommended Weapon, Tradeoff y, cuando aplica, requisito/progreso exacto de Contract. La frontal aprobada sigue siendo `ref-field-engineer-front-v1.png`; Field Repair mantiene su 1% config-backed, Bolt Cannon es solo **Recommended Weapon** y el tradeoff declara `-5% Damage`. No se recorta, trunca ni oculta ningún dato player-facing. A 1920×1080 y 1024×600 el contenido actual cabe sin scrollbar; ventanas más estrechas/cortas, copy futuro o roster de varias filas degradan a dos y una columna y a un único scroll vertical de `.character-layout`. `.character-grid` y el perfil permanecen `overflow: visible`; Back/Confirm quedan fuera y siempre visibles. Teclado, mouse, XInput/DirectInput e Interact remapeado conservan el handoff en los límites; si no existe rango de scroll, la navegación pasa directamente a las acciones. No se crea canvas, observer, RAF ni WebGL, y los contratos `data-character-*` permanecen estables.
