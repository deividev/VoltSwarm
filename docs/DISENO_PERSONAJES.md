...
- `Proving Ground` y `Run Completion` existen solo como definiciones latentes y excluidas de la UI/evaluación. Ambos conservan internamente `next-core`; `Run Completion` evalúa runs completadas y no rastrea personajes distintos. No son desbloqueos de personaje implementados.
