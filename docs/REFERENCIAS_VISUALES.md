...
9. **Modelos voxel IA** — ✅ Pipeline 2D→3D congelado y validado: 6 enemigos + bosses in-game + jugador, todos vía `src/models/registry.ts`. Hazard Marshal es el boss final vigente del juego completo (modelo elegido; combate provisional). Volt Warden queda como diseño histórico/futuro, no como boss final. 121 FPS validados con el elenco completo + bloom activo.
