...
- **Sockets** (estado de cuenta, se amplían por Contratos de Desguace): armas **2 de inicio → 3** · cores **2 de inicio → 4**. **Sin swap en v1**: un core instalado es compromiso de todo el run. Boss Hunter abre el tercer socket de arma; Second Wind y Level Milestone abren los dos de core.
...
**Rareza del draft (recalibrada 2026-07-17; experimento 2026-08-04):** pesos a `Luck = 0%` gris 62 · verde 27 · azul 9 · morado 1.8 · dorado 0.2. Con tres cartas, la probabilidad inicial de ver morado/dorado es `5.881%`, y la de ver dorado es `0.599%`. Luck se guarda y muestra como rating porcentual (`0.10` = `10%`), pero NO suma puntos porcentuales directos a una probabilidad: desplaza los pesos azul/morado/dorado y después se normaliza el pool. Para el próximo playtest humano de la demo, la primera copia Common de Lucky Gear es `+4%` (antes `+6%`) y lleva esas probabilidades a aproximadamente `11.633%` y `2.857%`; es el único número de balance modificado. Luck base permanece en `0%`, los tiers superiores en `8/10/14/20%`, y pesos/`luckShift` no cambian.
